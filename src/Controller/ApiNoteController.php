...
class ApiNoteController extends AbstractController{
    ...
    public function getAllNotes(NoteRepository $repo, LivreRepository $repoBook,String $id):Response{
    ...
    #[Route('/api/note/delete/{id}',name:'app_api_note_delete',methods:'DELETE')]
    public function deleteNote(NoteRepository $repo,LivreRepository $repoBook,EntityManagerInterface $em,int $id):Response{
        try{
            $note = $repo->find($id);
            if(!$note){
                return $this->json(['erreur'=>'La note n\'existe pas en BDD'], 400, 
                ['Content-Type'=>'application/json',
                'Access-Control-Allow-Origin'=> 'localhost',
                'Access-Control-Allow-Methods'=> 'GET'],[]);
            }
            $em->remove($note);
            $em->flush();
            return $this->json(['erreur'=>'La note '.$note->getId().' a été supprimé en BDD'], 200, 
            ['Content-Type'=>'application/json',
            'Access-Control-Allow-Origin'=> 'localhost',
            'Access-Control-Allow-Methods'=> 'GET'],[]);
        }
    ...
    #[Route('/api/note/update/{id}',name:'app_api_note_update',methods:'UPDATE')]
    public function updateNote(NoteRepository $repo,Request $request,EntityManagerInterface $em, SerializerInterface $serialize):Response{
        try{
            $json = $request->getContent();
            ...
            if(empty($data['score']) OR empty($data['date']) OR empty($data['critique']) OR empty($data['titreCritique'])){
                return $this->json(['erreur'=>'Veuillez remplir les valeurs'], 400, 
                ['Content-Type'=>'application/json',
                'Access-Control-Allow-Origin'=> 'localhost',
                'Access-Control-Allow-Methods'=> 'GET'],[]);
            }
            $note = $repo->find($data['id'])
            // date,score,critique,titrecritique
        }
    }
}